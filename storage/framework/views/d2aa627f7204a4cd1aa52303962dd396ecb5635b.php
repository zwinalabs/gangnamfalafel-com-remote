...
<!-- Circles background -->
<?php echo $__env->make('restorants.partials.landingpage', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
        ...
        <div class="container-fluid container-restorant">
            ...
            <div class="row">
                ...
                <?php if(!$restorant->categories->isEmpty()): ?>
                    <nav class="tabbable sticky" style="/*top: <?php echo e(config('app.isqrsaas') ? 75:88); ?>px;*/">
                        <ul id="categories-items" class="nav nav-pills bg-white mb-2">
                            <li class="nav-item nav-item-category ">
                                <a class="nav-link  mb-sm-3 mb-md-0 active" data-toggle="tab" role="tab" href="" onClick="closeCurrentItem()"><?php echo e(__('All categories')); ?></a>
                            </li>
                            <?php $__currentLoopData = $restorant->categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if(!$category->aitems->isEmpty()): ?>
                                    <li class="nav-item nav-item-category" id="<?php echo e('cat_'.clean(str_replace(' ', '', strtolower($category->name)).strval($key))); ?>">
                                        <a class="nav-link mb-sm-3 mb-md-0" data-toggle="tab" role="tab" onClick="closeCurrentItem()" id="<?php echo e('nav_'.clean(str_replace(' ', '', strtolower($category->name)).strval($key))); ?>" href="#<?php echo e(clean(str_replace(' ', '', strtolower($category->name)).strval($key))); ?>"><?php echo e($category->name); ?></a>
                                    </li>
                                <?php endif; ?>
                ...
                <div id="restorant-menu-listing" class="col-12  col-lg-7 borne-menu-listing">
                    <?php if(!$restorant->categories->isEmpty()): ?>
                            ...
                            <?php $__currentLoopData = $restorant->categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <div class="row <?php echo e(clean(str_replace(' ', '', strtolower($category->name)).strval($key))); ?>">
                                    <?php $__currentLoopData = $category->aitems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div id="item-listing-<?php echo e($item->id); ?>" class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-6 pl-9px pr-9px">
                                            <div class="strip">
                                                <?php if(!empty($item->image)): ?>
                                                <figure>
                                                    <a onClick="setCurrentItem(<?php echo e($item->id); ?>);openCurrentItem(<?php echo e($item->id); ?>);" href="javascript:void(0)">
                                                ...
                                                <?php endif; ?>
                                                <input type="hidden" id="currentItem-<?php echo e($item->id); ?>" value="<?php echo e($item->id); ?>"/>
                                                <input type="hidden" id="itemName-<?php echo e($item->id); ?>" value="<?php echo e($item->name); ?>"/>
                                                <input type="hidden" id="itemPrice-<?php echo e($item->id); ?>" value="<?php echo e($item->price); ?>"/>
                                                <input type="hidden" id="itemUnix-<?php echo e($item->id); ?>" value=""/>
                                                <input type="hidden" id="itemFriendlyPrice-<?php echo e($item->id); ?>" value="<?php echo money($item->price, config('settings.cashier_currency'),config('settings.do_convertion')); ?>"/>
                                                <div class="res_title"><a onClick="setCurrentItem(<?php echo e($item->id); ?>);openCurrentItem(<?php echo e($item->id); ?>);" href="javascript:void(0)"><?php echo e($item->name); ?></a></div>
                                                    ...
                                                    <div class="col-7 pr-0">
                                                        <?php if(!(isset($canDoOrdering) && !$canDoOrdering)): ?>
                                                            <div class="row p-l-none flex-right-direction">
                                                                <div id="addToCartExpress-<?php echo e($item->id); ?>"
                                                                    class="add-to-cart-express-<?php echo e($item->id); ?> order-0 text-center">
                                                                    <button type="button" id="btn-add-tocart-express-<?php echo e($item->id); ?>" onClick='addToCartActExpress(<?php echo e($item->id); ?>)' :value="item.id"
                                                            ...
                                                            <div class="row quantity-express-bloc p-l-none flex-right-direction hide-me" id="quantity-express-bloc-<?php echo e($item->id); ?>" style="display:none">
                                                                <button id="list-express-trash-<?php echo e($item->id); ?>" type="button" onClick="removeItemFromCartByListing(<?php echo e($item->id); ?>)" class="btn btn-outline-primary btn-icon btn-sm page-link btn-cart-radius">
                                                                ...
                                                                <button id="list-express-minus-<?php echo e($item->id); ?>" type="button" onClick="decQuantityExpress(<?php echo e($item->id); ?>)"
                                                                ...
                                                                <input type="number" min="1" step="1"
                                                                    ...
                                                                    name="quantityExpress-<?php echo e($item->id); ?>" id="quantityExpress-<?php echo e($item->id); ?>"
                                                                ...
                                                                <button id="list-express-plus-<?php echo e($item->id); ?>" type="button" onClick="incQuantityExpress(<?php echo e($item->id); ?>)"
                                                                    class="btn btn-outline-primary btn-icon btn-sm page-link btn-cart-radius">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" fill="currentColor" data-spec="icon-plus" class="w-5 h-5" style="width: 16px; height: 16px;"><path fill="currentColor" d="M376 232H216V72c0-4.42-3.58-8-8-8h-32c-4.42 0-8 3.58-8 8v160H8c-4.42 0-8 3.58-8 8v32c0 4.42 3.58 8 8 8h160v160c0 4.42 3.58 8 8 8h32c4.42 0 8-3.58 8-8V280h160c4.42 0 8-3.58 8-8v-32c0-4.42-3.58-8-8-8z"></path></svg>
                                                                </button>
                                                            </div>
                        ...
                        <div id="productFullDetails" class="row" style="display:none">
                            <div class="full-modal-body p-0">
                                <div class="card border-0">
                                    <div class="card-body px-lg-5 py-lg-5 pl-0 pr-0">
                                        <div id="fullItemDisplayer">
                ...
                <div id="borne-cart" class="col-12 col-lg-3 borne-cart">
                    <?php echo $__env->make('restorants.partials.carts', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                </div>
            </div>
        ...
        <?php if(  !(isset($canDoOrdering)&&!$canDoOrdering)   ): ?>
            <div onClick="openNav()" class="callOutShoppingButtonBottom icon icon-shape bg-gradient-red text-white rounded-circle shadow mb-1">
                <div class="row col-12">
                    ...
                    <div class="col-5 text-right pl-0 pr-0" id="link-footer-basket" ><?php echo e(__('View basket')); ?></div>
...
<?php if($showLanguagesSelector): ?>
    ...
    <?php $__env->stopSection(); ?>
<?php endif; ?>
<div id="cartListMobile" class="d-none"></div>
<div id="totalSubmit" class="d-none"></div>

<?php
    session(['isBorne' => false]);  //elle sera une variable configurable en backend donc administrable !
?>
<?php $__env->startSection('js'); ?>
    ...
    <script>
        ...
        var DISPLAY_CADDY = "";
        var STRIPE_INTENTS = "<?php echo e(route('stripe.postPaymentIntents')); ?>";
        var STRIPE_CHARGES = "<?php echo e(route('stripe.retrievePayment')); ?>";
    </script>
    <script src="<?php echo e(asset('custom')); ?>/js/apple-pay.js"></script>
...
<?php echo $__env->make('layouts.front', ['class' => ''], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\gangnamfalafel-com\resources\views/restorants/show.blade.php ENDPATH**/ ?>