...
class CartController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        if ($item && $canAdd) {
            ...
            if($request->extras && !is_scalar($request->extras)){
            ...
            Cart::add($request->timesTamp, $cartItemName, $cartItemPrice, $request->quantity, ['id'=>$item->id, 'variant'=>$request->variantID, 'extras'=>$request->extras, 'restorant_id'=>$restID, 'image'=>$item->icon, 'friendly_price'=>  Money($cartItemPrice, config('settings.cashier_currency'), config('settings.do_convertion'))->format()]);
    ...
    public function addBulk(Request $request){
        ...
        $datas = json_decode(json_encode($request->data));
        $returnDammy = [];
        foreach ($datas as $key => $cItem) {
            $item = Items::find($cItem->attributes->id);
            $restID = $item->category->restorant->id;

            $restaurant = Restorant::findOrFail($restID);
            \App\Services\ConfChanger::switchCurrency($restaurant);
            
            //if item is deleted from caddy
            /*if(in_array($cItem->id, $datas->deletedList)){
                Cart::remove($cItem->id);
            }*/
            ...
            if ($item && $canAdd) {
                ...
                //variantID
                if ($cItem->attributes->variant) {
                    //Get the variant
                    $variant = Variants::findOrFail($cItem->attributes->variant);
                ...
                if($cItem->attributes->extras && !is_scalar($cItem->attributes->extras)){
                    foreach ($cItem->attributes->extras as $key => $value) {
                        $cartItemName .= "\n+ ".$item->extras()->findOrFail($value)->name;
                        $cartItemPrice += $item->extras()->findOrFail($value)->price;
                        $theElement .= $value.' -- '.$item->extras()->findOrFail($value)->name.'  --> '.$cartItemPrice.' ->- ';
                    }
                ...
                if($cItem->remote === 0){
                    Cart::add(
                        ...
                        $cItem->quantity, 
                        ['id'=>$item->id, 
                        'variant'=>$cItem->attributes->variant, 
                        'extras'=>$cItem->attributes->extras, 
                        'restorant_id'=>$restID, 
                        'image'=>$item->icon, 
                        'friendly_price'=>  Money($cartItemPrice, config('settings.cashier_currency'), config('settings.do_convertion'))->format()]
                    );
                    $returnDammy[$item->id] = [
                        'status' => true,
                        'errMsg' => $theElement,
                    ...
                    $this->updateCartQtyNotRelative($cItem->quantity,$cItem->id);
                    $returnDammy[$item->id] = [
                        'status' => true,
                        'errMsg' => $theElement,
                        'ftn' => 'updateCartQtyNotRelative',
                        'qtyyyy' => $cItem->quantity,
                        '$cItem->id' => $cItem->id
                    ];
                }
            }else{
        ...
        return response()->json($returnDammy);
    }
    ...
    public function cartApiResponse()
    {
        ...
        $itemsCountTotalPrice = Cart::getSubTotal();
        if(!$isEmpty){
            //Cart is not empty
            $restorantID = null;
            foreach (Cart::getContent() as $key => $cartItem) {
        ...
        //return view('cart.cartcheckouts', $params)->withError(__('Your cart is empty!'));
    }
    ...
    private function updateCartQtyNotRelative($howMuch, $item_id)
    {
        if(isset($_GET['session_id'])){
            $this->setSessionID($_GET['session_id']);
        }

        $return = Cart::update($item_id, array('quantity' => array('relative' => false, 'value' => (int)$howMuch)));
