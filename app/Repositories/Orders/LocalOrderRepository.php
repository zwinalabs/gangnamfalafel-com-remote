...
class LocalOrderRepository extends BaseOrderRepository implements OrderTypeInterface
{
    ...
    public function redirectOrInform($dataStripe=[]){
        if($this->status){
            if($this->paymentRedirect==null){
                //We don't have payment redirects
                if($this->isNewOrder){
                    //New order - redirect to success page
                    $data = ['order' => $this->order, 'stripe_links' => $dataStripe['stripe_links']];
                    
                    //if isBorne
                        $this->updateOrderBorne("STRIPE", $dataStripe['hiboutik_sale_id'] );
