<div class="card-profile">
    <div class="">
      <div class="mt-3">
        {{--<h3>{{ __('Order Summary') }}<span class="font-weight-light"></span></h3>
      </div>--}}
        <!-- List of items -->
        <div  id="cartList" class="border-top pt-3 d-none">

            <div  v-for="item in items" class="items col-xs-12 col-sm-12 col-md-12 col-lg-12 clearfix  pl-0">
                <div v-if="itemsCount!=0" class="info-block block-info clearfix" v-cloak>
                    ...
                    <div class="row m-r-0">
                        ...
                        <div class="row col-7 text-center flex-right-direction pl-0 pr-0">
                            <input :id="item.attributes.id" type="hidden" :name="item.attributes.id" :value="item.id">
                            <input :id="'checkout-qty-span-ipt-'+item.id" type="hidden" :name="'checkout-quantity-span-ipt-'+item.id" :value="item.quantity">

                            <input :id="'checkout-item-'+item.id" type="hidden" :name="'checkout-item-'+item.id" :value="item.attributes.id">
                        
                            <button v-show="item.quantity <= 1" :id="'cart-item-btns-trash-' + item.id" type="button" v-on:click="removeCh(item.id)"  :value="item.id" class="btn btn-outline-primary btn-icon btn-sm page-link btn-cart-radius">
                            ...
                            <button v-show="item.quantity > 1" :id="'cart-item-btns-minus-' + item.id" type="button" v-on:click="decCartCh(item.id)" :value="item.id" class="btn btn-outline-primary btn-icon btn-sm page-link btn-cart-radius" style="display: none">
                            ...
                            <span id="'checkout-quantity-span'+item.id" class="checkout-quantity-span">@{{ item.quantity }}</span>
                            <button :id="'cart-item-btns-plus-' + item.id" type="button"  v-on:click="incCartCh(item.id)"  :value="item.id" class="btn btn-outline-primary btn-icon btn-sm page-link btn-cart-radius">
                ...
                <div v-if="itemsCount==0" class="info-block block-info clearfix" v-cloak>
                    <div v-if="itemsCount==0" class="text-center" >
                        <svg fill="#089151" height="32px" width="32px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 483.1 483.1" xml:space="preserve" stroke="#089151"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M434.55,418.7l-27.8-313.3c-0.5-6.2-5.7-10.9-12-10.9h-58.6c-0.1-52.1-42.5-94.5-94.6-94.5s-94.5,42.4-94.6,94.5h-58.6 c-6.2,0-11.4,4.7-12,10.9l-27.8,313.3c0,0.4,0,0.7,0,1.1c0,34.9,32.1,63.3,71.5,63.3h243c39.4,0,71.5-28.4,71.5-63.3 C434.55,419.4,434.55,419.1,434.55,418.7z M241.55,24c38.9,0,70.5,31.6,70.6,70.5h-141.2C171.05,55.6,202.65,24,241.55,24z M363.05,459h-243c-26,0-47.2-17.3-47.5-38.8l26.8-301.7h47.6v42.1c0,6.6,5.4,12,12,12s12-5.4,12-12v-42.1h141.2v42.1 c0,6.6,5.4,12,12,12s12-5.4,12-12v-42.1h47.6l26.8,301.8C410.25,441.7,389.05,459,363.05,459z"></path> </g> </g></svg>
                    </div> 
                    <div v-if="itemsCount==0" ><span v-if="itemsCount==0">{{ __('Cart is empty') }}!</span></div>
                    <div v-if="itemsCount==0" class="text-center">
                        <a href="{{ route('vendor',$restorant->subdomain) }}" type="button" class="btn btn-default btn-block text-white btn-sm" style="text-transform:none" >{{ __('Continue Shopping') }}</a>
                    </div>
                </div>
            </div>
        </div>
