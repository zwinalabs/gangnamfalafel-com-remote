...
<div class="modal fade" id="modal-import-items-hiboutik" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
    <div class="modal-dialog modal- modal-dialog-centered modal-" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="modal-title-new-item">{{ __('Import items from Hiboutik API') }}</h3>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="modal-body p-0">
                <div class="card bg-secondary shadow border-0">
                    <div class="card-body px-lg-5 py-lg-5">
                        <div class="col-md-10 offset-md-1">
                        {{-- <form role="form" method="post" action="{{ route('import.items-hiboutik') }}" enctype="multipart/form-data"> --}}
                            @csrf
                            <div class="form-group text-center{{ $errors->has('item_image') ? ' has-danger' : '' }}">
                                {{-- <label class="form-control-label" for="items_excel">{{ __('Import your file') }}</label> --}}
                                <div class="text-center">
                            ...
                            <input name="res_id" id="res_id" type="hidden" value="{{ $restorant_id }}" required>
                            <div class="text-center">
                                <button type="button" onclick="return confirm('{{ __('The import will delete all your local items. Are you sure you want to replace local products by items from Hiboutik?') }}');" class="btn btn-primary my-4">{{ __('Start Importing items from Hiboutik') }}</button>
                            </div>
                        {{-- </form>  --}}
                        </div>
                    </div>
